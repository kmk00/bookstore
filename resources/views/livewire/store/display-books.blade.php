<main class="flex-grow-[999] basis-4/6 p-2">
    <div
        class="grid grid-cols-[repeat(auto-fit,minmax(190px,1fr))] md:grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-4 p-2">
            ...
            <div class="flex flex-col hover:scale-105 transition duration-100 ease-in h-full shadow-md p-2 justify-between">
                ...
                @if ($book->quantityAvailable > 1)
                    <button :disabled="{{ $book->quantityAvailable < 1 }}" x-data="{ clicked: false }"
                        ...
                        class="w-fit mt-2 bg-primary active:bg-green-500 shadow-md shadow-black/20 transition duration-100 ease-in text-white font-bold py-2 px-4 rounded"
